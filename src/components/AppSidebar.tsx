...
export function AppSidebar({ onAddTask, currentFilter, onFilterChange }: AppSidebarProps) {
  ...
  const renderNavItem = (item: NavItemConfig, index: number) => {
    const isActive = item.isFilter ? item.filterValue === currentFilter : item.isPageLink ? pathname === item.href : false;
  ...
  if (authLoading || !user) {
    // Render a minimal sidebar or nothing if user is not logged in or auth is loading
    // This matches the behavior of not showing AppSidebar if !user in page.tsx
    return null;
  }
      ...
      <SidebarHeader className="p-3 h-[60px] border-b border-sidebar-border flex items-center gap-2">
        ...
        {!isIconOnly && (
          <Link href="/" className="flex items-center gap-2 group overflow-hidden">
            <Rocket className="h-6 w-6 text-sidebar-primary group-hover:text-sidebar-primary/90 transition-colors shrink-0" />
      ...
      <SidebarContent className="p-0"> 
        <ScrollArea className="h-full w-full">
          <div className="p-2 space-y-1"> 
            ...
            {!isIconOnly && (
              <div className="px-3 pt-2 pb-1 mt-2 text-xs font-semibold uppercase text-sidebar-foreground/70 tracking-wider">
                Filters
              </div>
            )}
            <SidebarMenu>{filterNavItems.map(renderNavItem)}</SidebarMenu>
